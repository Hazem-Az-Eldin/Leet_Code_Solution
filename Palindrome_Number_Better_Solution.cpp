...
#include<vector>
// Orginal SOlution  
//1) transfer int numb to arry or vector (Unessacry increase the run time)
//2) check if the vector is equal to the number
//3) check if number is negative then add to the begining 
//4) check if the number  

// Better Solution
// 1) take the reminder into a new value instead of a vector 
// 2) do the checks on the new value instead of two loops 
class Solution {
public:
